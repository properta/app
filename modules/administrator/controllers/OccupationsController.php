...
use app\models\mains\{
    generals\MOccupations,
    generals\Settings,
    searches\MOccupations as MOccupationsSearch
};
use app\utils\{
    gdrive\GDrive
};
...
class OccupationsController extends Controller
{
    ...
    {
        if (Yii::$app->user->can('/occupations/index') || 1) :
            $searchModel = new MOccupationsSearch();
    ...
    {
        if (Yii::$app->user->can('/occupations/create') || 1) :
            $model = new MOccupations();
            $msg = "";
    ...
    public function actionValidate($code = '')
    {
        ...
        if ($code) :
            ...
            $model = MOccupations::findOne($code);
        endif;
        if ($model->load(Yii::$app->request->post())) :
            Yii::$app->response->format = Response::FORMAT_JSON;
    ...
    {
        if (Yii::$app->user->can('/occupations/view') || 1) :
            $code = Yii::$app->encryptor->decodeUrl($code);
            $model = $this->findModel($code);
            $msg = "";
            if ($model->load(Yii::$app->request->post())) :
    ...
    {
        Yii::$app->response->format = Response::FORMAT_JSON;
        if (Yii::$app->user->can('/occupations/delete') || 1) :
    ...
    {
        $model = MOccupations::find()->where(['id' => $id])
            ->andWhere(['deleted_at' => NULL])
            ->one();
        if ($model !== null) :
            return $model;
