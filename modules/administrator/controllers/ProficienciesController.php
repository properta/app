...
use app\models\mains\{
    generals\MProficiencies,
    generals\Settings,
    searches\MProficiencies as MProficienciesSearch
};
use app\utils\{
    gdrive\GDrive
};
...
class ProficienciesController extends Controller
{
    ...
    {
        if (Yii::$app->user->can('/proficiencies/index') || 1) :
            $searchModel = new MProficienciesSearch();
    ...
    {
        if (Yii::$app->user->can('/proficiencies/create') || 1) :
            $model = new MProficiencies();
            $msg = "";
    ...
    public function actionValidate($code = '')
    {
        ...
        if ($code) :
            ...
            $model = MProficiencies::findOne($code);
        endif;
        if ($model->load(Yii::$app->request->post())) :
            Yii::$app->response->format = Response::FORMAT_JSON;
    ...
    {
        if (Yii::$app->user->can('/proficiencies/view') || 1) :
            $code = Yii::$app->encryptor->decodeUrl($code);
            $model = $this->findModel($code);
            $msg = "";
            if ($model->load(Yii::$app->request->post())) :
    ...
    {
        Yii::$app->response->format = Response::FORMAT_JSON;
        if (Yii::$app->user->can('/proficiencies/delete') || 1) :
    ...
    {
        $model = MProficiencies::find()->where(['id' => $id])
            ->andWhere(['deleted_at' => NULL])
            ->one();
        if ($model !== null) :
            return $model;
