        ...
        <div class="card">
            <div class="card-body">
                <div class="form">
                    ...
                    <?= $form->field($model, 'campus_str')
                    ->dropDownList($campuses??[], ['class'=>'form-control get-campuses select2', 'value'=>isset($campuses)?array_keys($campuses):[], 'prompt'=>'--choose one--'])->label("campus Name"); ?>

                    <?= $form->field($model, 'campus_website_str', Template::template('fas fa-link'))->textInput(['maxlength' => true, 'placeholder' =>'https://stieriau-akbar.ac.id/'])->label('Title') ?>

                    <?= $form->field($model, 'campus_address_str')->textArea(['maxlength' => true, 'placeholder' =>'', 'class' =>'form-control'])->label('campus Address') ?>
...
function init(){
    ...
    $('.get-campuses').change(function () {
       $.ajax({
           url: 'get-campus-address',
           dataType: 'JSON',
           data: {
               campus_name: $(this).val(),
               _csrf: _csrf
            },
            success: function (data) {
                let {result} = data;
                $('#pmbinformations-campus_address_str').val(result)
            }
       })

       $.ajax({
           url: 'get-campus-website',
           dataType: 'JSON',
           data: {
               campus_name: $(this).val(),
               _csrf: _csrf
            ...
            success: function (data) {
                let {result} = data;
                $('#pmbinformations-campus_website_str').val(result)
            }
       })
    })
